...
func TestUDPConnect(t *testing.T) {
	mock := &NetConnMock{
		values: map[string]interface{}{},
		writeFn: func(mock NetConnMock, b []byte) (int, error) {
			txID := binary.BigEndian.Uint32(b[12:])
			mock.values["txID"] = txID
			return 0, nil
		},
		readFn: func(mock NetConnMock, dst []byte) (int, error) {
			txID, _ := mock.values["txID"].(uint32)

			response, _ := tracker.ConnectResp{
	...
	connID, err := tracker.UDPConnect(url, mock)
